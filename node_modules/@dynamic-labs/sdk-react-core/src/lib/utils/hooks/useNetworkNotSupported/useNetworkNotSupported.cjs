...
const useNetworkNotSupported = () => {
    const { selectedWalletConnector: walletConnector, setShowAuthFlow, bridgeChainsToConnect, bridgeOnboardingCompleted, isBridgeFlow, user, } = useInternalDynamicContext.useInternalDynamicContext();
    ...
    const { network: walletConnectorNetwork } = useWalletConnectorNetwork.useWalletConnectorNetwork(walletConnector, {
        onChange: (walletConnectorNetwork) => {
            ...
            finishConnectAndSign({
                ...
                if (!user && authMode$1 === 'connect-and-sign') {
                    return;
                }
