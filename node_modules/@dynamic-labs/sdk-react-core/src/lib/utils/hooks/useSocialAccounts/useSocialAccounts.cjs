...
const useSocialAccounts = ({ onError } = {}) => {
    ...
    const [processingStates, setProcessingStates] = React.useState(new Map());
    // Helper functions for provider-specific processing state
    const setProcessingForProvider = React.useCallback((provider, isProcessing) => {
        setProcessingStates((prev) => new Map(prev).set(provider, isProcessing));
    }, []);
    const isProcessingForProvider = React.useCallback((provider) => { var _a; return (_a = processingStates.get(provider)) !== null && _a !== void 0 ? _a : false; }, [processingStates]);
    const handleFarcasterUrl = React.useCallback((url) => pushView('farcaster-connect-view', { url }, {
        onBackClick: () => {
    ...
    const findVerifiedCredentialById = React.useCallback((provider, verifiedCredentialId) => {
        var _a;
    ...
    const isProcessingGlobal = React.useMemo(() => Array.from(processingStates.values()).some(Boolean), [processingStates]);
