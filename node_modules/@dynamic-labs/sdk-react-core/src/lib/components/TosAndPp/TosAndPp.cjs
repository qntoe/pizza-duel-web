...
const TosAndPp = () => {
    const { customPrivacyPolicy, customTermsOfServices, privacyPolicyUrl, termsOfServiceUrl, } = useInternalDynamicContext.useInternalDynamicContext();
    const connectionType = isConnectOnly.isConnectOnly() ? 'connecting' : 'logging in';
