...
const PasskeyBundleRecovery = ({ authenticatorType, currentAction, }) => {
    const { primaryWallet, environmentId, user, setShowAuthFlow } = useInternalDynamicContext.useInternalDynamicContext();
    ...
    const closeButton = (jsxRuntime.jsx(IconButton.IconButton, { type: 'button', onClick: () => {
            setShowAuthFlow(false, { emitCancelAuth: true });
            dynamicEvents.dynamicEvents.emit('passkeyRecoveryFailed', new utils.DynamicError('Validation request cancelled'));
        }, "data-testid": 'close-button', children: jsxRuntime.jsx(close.ReactComponent, {}) }));
