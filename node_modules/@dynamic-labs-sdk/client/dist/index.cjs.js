const require_getNetworkProviderFromNetworkId = require('./getNetworkProviderFromNetworkId-DCZQS4ot.cjs.js');
...
const require_isMfaRequiredForAction = require('./isMfaRequiredForAction-BeZstT1P.cjs.js');
const require_NotWaasWalletAccountError = require('./NotWaasWalletAccountError-Dq2ssnPs.cjs.js');
...
*/
var InvalidRetryOnFailCallError = class extends require_constants.BaseError {
...
*/
const isSignedIn = (client = require_constants.getDefaultClient()) => Boolean(client.user || require_getVerifiedCredentialForWalletAccount.getWalletAccounts(client).length > 0);
...
*/
const fetchProjectSettings = async (client = require_constants.getDefaultClient()) => {
...
const generateSessionKeys = async (client) => {
	...
	const encodedSessionKeys = require_constants.getBuffer().from(stringifiedSessionKeys).toString("base64");
...
*/
const logout = async (client = require_constants.getDefaultClient()) => {
	const core = require_constants.getCore(client);
	...
	if (client.user !== null) {
		...
		*/
		if (require_constants.isCookieEnabled(client)) require_getVerifiedCredentialForWalletAccount.setCookie(`${require_getVerifiedCredentialForWalletAccount.DYNAMIC_AUTH_COOKIE_NAME}=; Max-Age=-99999999; path=/; SameSite=Lax`);
	}
	core.state.set({
		captchaToken: null,
		legacyToken: null,
...
const raiseStateEvents = (client) => {
	require_constants.getCore(client).state.subscribe((value, previous) => {
...
const unverifiedWalletAccountSchema = zod_mini.object({
	...
	addressesWithTypes: zod_mini.optional(zod_mini.array(zod_mini.object({
		address: zod_mini.string(),
		publicKey: zod_mini.optional(zod_mini.string()),
...
const walletProviderKeyMapSchema = zod_mini.record(zod_mini.string(), zod_mini.string());
...
const sessionStorageKeySchema = require_getNetworkProviderFromNetworkId.createStorageKeySchema({
	...
	schema: zod_mini.object({
		...
		sessionKeys: zod_mini.nullable(zod_mini.string()),
		token: zod_mini.nullable(zod_mini.string())
	})
});
...
const hydrateStateWithStorage = async (client) => {
	...
	if (storedState?.apiVersion === require_constants.SDK_API_CORE_VERSION) {
...
const syncStateWithStorage = (client) => {
	...
	core.state.subscribe((state) => {
		...
		core.storage.setItem(stateStorageKeySchema, {
			apiVersion: require_constants.SDK_API_CORE_VERSION,
...
var ClientAlreadyInitializedError = class extends require_constants.BaseError {
	constructor() {
		super({
			cause: null,
...
*/
const initializeClient = async (client = require_constants.getDefaultClient()) => {
	const core = require_constants.getCore(client);
	...
	*/
	...
	core.initTrack.track({
		name: require_getNetworkProviderFromNetworkId.FETCH_PROJECT_SETTINGS_TRACKER_KEY,
		promise: fetchProjectSettingsPromise
	});
	core.initTrack.track({
		name: require_getNetworkProviderFromNetworkId.GENERATE_SESSION_KEYS_TRACKER_KEY,
...
const createAsyncTrack = () => {
	...
	*/
	...
	const track = ({ name: name$1, promise }) => {
		if (!isTrackEnabled) throw new require_getNetworkProviderFromNetworkId.CannotTrackError();
...
var UnavailableInServerSideError = class extends require_constants.BaseError {
...
const createDebouncedMutex = () => {
	...
	return async ({ lockKey, callback, debounceTime = 0 }) => {
		...
		require_constants.assertDefined(currentState, `Mutex state for key ${lockKey} not found`);
...
var InvalidStorageValue = class extends require_constants.BaseError {
...
const createCore = (config) => {
	...
	return {
		...
		state,
		storage,
		version: require_constants.version
	};
};
...
const createDynamicClient = (config) => {
	...
	require_constants.setDefaultClient(client);
	return client;
};

//#endregion
//#region src/errors/InvalidDeviceRegistrationRedirectError.ts
var InvalidDeviceRegistrationRedirectError = class extends require_constants.BaseError {
	constructor() {
		super({
			cause: null,
...
var InvalidRedirectStorageStateError = class extends require_constants.BaseError {
	constructor() {
		super({
			cause: null,
...
var MissingRedirectStorageStateError = class extends require_constants.BaseError {
	constructor() {
		super({
			cause: null,
...
var MissingSocialUrlParamError = class extends require_constants.BaseError {
	constructor(param) {
		super({
			cause: null,
...
var MissingUserVerificationError = class extends require_constants.BaseError {
	informationToVerify;
...
var NoAddressFoundError = class extends require_constants.BaseError {
	constructor() {
		super({
			cause: null,
...
var NoPasskeyCredentialsFoundError = class extends require_constants.BaseError {
	constructor() {
		super({
			cause: null,
...
var NoSmartWalletAccountSignerFoundError = class extends require_constants.BaseError {
...
var NoWebAuthNSupportError = class extends require_constants.BaseError {
	constructor() {
		super({
			cause: null,
...
var UnrecognizedNetworkError = class extends require_constants.BaseError {
...
var UserNotAuthenticatedError = class extends require_constants.BaseError {
...
var UserRejectedError = class extends require_constants.BaseError {
	constructor({ action }) {
		super({
			cause: null,
			code: "user_rejected",
...
var WalletAccountAlreadyVerifiedError = class extends require_constants.BaseError {
	constructor(address) {
		super({
			cause: null,
...
var WalletAccountNotSelectedError = class extends require_constants.BaseError {
	expectedAddress;
	selectedAddress;
	constructor(params) {
...
var WalletProviderMethodUnavailableError = class extends require_constants.BaseError {
...
*/
const signInWithExternalJwt = async ({ externalJwt, sessionPublicKey } = {}, client = require_constants.getDefaultClient()) => {
	const core = require_constants.getCore(client);
	const response = await require_constants.createApiClient({}, client).externalAuthSignin({
		environmentId: core.environmentId,
...
*/
const deletePasskey = async ({ passkeyId }, client = require_constants.getDefaultClient()) => {
	const core = require_constants.getCore(client);
	await require_constants.createApiClient({ includeMfaToken: true }, client).deletePasskey({
...
*/
const getPasskeys = async (client = require_constants.getDefaultClient()) => {
	const core = require_constants.getCore(client);
	return (await require_constants.createApiClient({}, client).getUserPasskeys({ environmentId: core.environmentId })).passkeys;
};
...
const getPasskeyRegistrationOptions = async (client) => {
	const core = require_constants.getCore(client);
	return await require_constants.createApiClient({}, client).getPasskeyRegistrationOptions({ environmentId: core.environmentId });
};
...
const serverRegisterPasskey = async ({ registration, createMfaToken }, client) => {
	const core = require_constants.getCore(client);
	return await require_constants.createApiClient({}, client).registerPasskey({
		environmentId: core.environmentId,
...
*/
const registerPasskey = async ({ createMfaToken } = {}, client = require_constants.getDefaultClient()) => {
	const core = require_constants.getCore(client);
...
const getPasskeyAuthenticationSignInOptions = async ({ relatedOriginRpId } = {}, client) => {
	const core = require_constants.getCore(client);
	return await require_constants.createApiClient({}, client).getPasskeyAuthenticationSigninOptions({
		environmentId: core.environmentId,
		relatedOriginRpId
	});
};
...
const serverSigninPasskey = async ({ authentication, createMfaToken }, client) => {
	const core = require_constants.getCore(client);
	return await require_constants.createApiClient({}, client).signinWithPasskey({
		environmentId: core.environmentId,
		passkeyAuthRequest: {
			...authentication,
			createMfaToken,
...
* @throws NoWebAuthNSupportError If WebAuthn is not supported by the browser.
*/
const signInWithPasskey = async ({ relatedOriginRpId } = {}, client = require_constants.getDefaultClient()) => {
	const core = require_constants.getCore(client);
	if (!core.passkey.isSupported()) throw new NoWebAuthNSupportError();
...
*/
const getUserSocialAccounts = (client = require_constants.getDefaultClient()) => {
	const { user } = client;
	require_constants.assertDefined(user, "User is not authenticated");
...
const redirectStateStorageKeySchema = require_getNetworkProviderFromNetworkId.createStorageKeySchema({
	...
	schema: zod_mini.object({
		...
		provider: zod_mini.enum(_dynamic_labs_sdk_api_core.ProviderEnum),
		state: zod_mini.string()
	})
});
...
const addOAuthUrlParams = (provider, baseUrl, options = {}) => {
	...
	if (providerType === _dynamic_labs_sdk_api_core.ProviderEnum.Tiktok) {
...
const getOAuthBaseUrl = (provider) => {
	...
	if (providerType === _dynamic_labs_sdk_api_core.ProviderEnum.Telegram) {
		require_constants.assertDefined(provider.authorizationUrl, "Telegram authorization URL not found");
		return new URL(provider.authorizationUrl);
	}
	require_constants.assertDefined(provider.baseAuthUrl, "Base auth URL not found");
	if (providerType === _dynamic_labs_sdk_api_core.ProviderEnum.Shopify) {
		require_constants.assertDefined(provider.shopifyStore, "Shopify store value not found in provider");
...
const buildOAuthUrl = async (core, provider) => {
	...
	require_constants.assertDefined(projectSettings, "Project settings not found");
...
* @param [client] - The Dynamic client instance. Only required when using multiple Dynamic clients.
*/
const authenticateWithSocial = async ({ provider, redirectUrl }, client = require_constants.getDefaultClient()) => {
	const core = require_constants.getCore(client);
	const apiClient = require_constants.createApiClient({}, client);
	await core.initTrack.waitForAll();
...
*/
const completeSocialAuthentication = async ({ url }, client = require_constants.getDefaultClient()) => {
	const core = require_constants.getCore(client);
	const apiClient = require_constants.createApiClient({}, client);
	await core.initTrack.waitForAll();
...
*/
const detectOAuthRedirect = async ({ url }, client = require_constants.getDefaultClient()) => {
	const core = require_constants.getCore(client);
	const dynamicOauthState = url.searchParams.get("dynamicOauthState");
	const dynamicOauthCode = url.searchParams.get("dynamicOauthCode");
...
*/
const getPrimaryWalletAccount = (client = require_constants.getDefaultClient()) => {
...
*/
const unlinkSocialAccount = async ({ verifiedCredentialId }, client = require_constants.getDefaultClient()) => {
	const core = require_constants.getCore(client);
	const apiClient = require_constants.createApiClient({}, client);
...
*/
const getMultichainBalances = async ({ balanceRequest }, client = require_constants.getDefaultClient()) => {
	const core = require_constants.getCore(client);
	return (await require_constants.createApiClient({}, client).postMultichainAccountBalances({
		environmentId: core.environmentId,
...
*/
const isDeviceRegistrationRequired = (client = require_constants.getDefaultClient()) => {
	const user = client.user;
	require_constants.assertDefined(user, "User not logged in");
...
*/
const addCoinbaseOnrampOrderEventListener = ({ listener }, client = require_constants.getDefaultClient()) => {
	const core = require_constants.getCore(client);
	if (typeof window === "undefined") throw new require_constants.ValueMustBeDefinedError("Window is not available to listen for Coinbase onramp order events");
...
*/
const getMissingVerificationForCoinbaseOnrampOrder = ({ paymentMethod }, client = require_constants.getDefaultClient()) => {
	...
	const phoneVerifiedCredential = user.verifiedCredentials.find((credential) => credential.format === _dynamic_labs_sdk_api_core.JwtVerifiedCredentialFormatEnum.PhoneNumber);
...
*/
const createCoinbaseOnrampOrder = async (orderParams, client = require_constants.getDefaultClient()) => {
...
*/
const getCoinbaseBuyUrl = async (buyUrlParams, client = require_constants.getDefaultClient()) => {
	const core = require_constants.getCore(client);
	return await require_constants.createApiClient({ includeMfaToken: true }, client).generateCoinbaseOnrampBuyUrl({
...
*/
const createCryptoDotComPayment = async (paymentParams, client = require_constants.getDefaultClient()) => {
	const core = require_constants.getCore(client);
	return await require_constants.createApiClient({ includeMfaToken: true }, client).createCryptoDotComPayment({
...
*/
const waitForClientInitialized = async (client = require_constants.getDefaultClient()) => {
	await require_constants.getCore(client).initTrack.waitForAll();
};
...
const fetchWithTimeout = async (client) => {
	...
	try {
		...
		if (!response.ok) throw new require_constants.APIError(`Failed to fetch wallet book: ${response.status} ${response.statusText}`, "wallet_book_fetch_error", response.status);
...
const walletBookCacheStorageKeySchema = require_getNetworkProviderFromNetworkId.createStorageKeySchema({
	...
	schema: zod_mini.object({
		...
		wallets: zod_mini.record(zod_mini.string(), zod_mini.any())
	})
});
...
const getChainFromWalletSchema = ({ wallet, walletKey }) => {
	...
	if (wallet.chains && wallet.chains.length > 0) {
		...
		try {
			return require_constants.getChainFromVerifiedCredentialChain(chainPrefix);
		} catch {}
	}
	if (walletKey) {
...
*/
const getWalletConnectCatalog = async (client = require_constants.getDefaultClient()) => {
...
const extractChainFromNormalizedName = ({ normalizedWalletNameWithChain }) => {
	...
	const chainSuffixes = Object.keys(require_constants.CHAINS_INFO_MAP).map((chain) => chain.toLowerCase()).sort((a, b) => b.length - a.length);
...
*/
const getWalletConnectCatalogWalletByWalletProviderKey = async ({ walletProviderKey }, client = require_constants.getDefaultClient()) => {
...
*/
const updateUser = async ({ userFields }, client = require_constants.getDefaultClient()) => {
	const core = require_constants.getCore(client);
	const response = await require_constants.createApiClient({ includeMfaToken: true }, client).updateSelf({
		environmentId: core.environmentId,
		userFields
	});
	require_getVerifiedCredentialForWalletAccount.updateAuthFromVerifyResponse({ response }, client);
...
*/
const acknowledgeRecoveryCodes = async (client = require_constants.getDefaultClient()) => {
...
*/
const authenticateMfaRecoveryCode = async ({ code, createMfaTokenOptions }, client = require_constants.getDefaultClient()) => {
	const core = require_constants.getCore(client);
	const apiClient = require_constants.createApiClient({}, client);
	try {
...
const getPasskeyAuthenticationOptions = async ({ relatedOriginRpId } = {}, client) => {
	const core = require_constants.getCore(client);
	return await require_constants.createApiClient({}, client).getPasskeyAuthenticationOptions({
		environmentId: core.environmentId,
		relatedOriginRpId
	});
};
...
const serverAuthenticatePasskey = async ({ authentication, createMfaToken }, client) => {
	const core = require_constants.getCore(client);
	return await require_constants.createApiClient({}, client).authenticateMfaPasskeyDevice({
		environmentId: core.environmentId,
		passkeyAuthRequest: {
			...authentication,
			createMfaToken,
...
*/
const authenticatePasskeyMFA = async ({ createMfaToken, relatedOriginRpId } = {}, client = require_constants.getDefaultClient()) => {
	const core = require_constants.getCore(client);
	try {
		if (!core.passkey.isSupported()) throw new NoWebAuthNSupportError();
...
* @returns A promise that resolves to the MFA authentication response.
*/
const authenticateTotpMfaDevice = async ({ deviceId, code, createMfaTokenOptions }, client = require_constants.getDefaultClient()) => {
	const core = require_constants.getCore(client);
	const apiClient = require_constants.createApiClient({}, client);
	try {
...
*/
const createNewMfaRecoveryCodes = async (client = require_constants.getDefaultClient()) => {
	const core = require_constants.getCore(client);
	return require_constants.createApiClient({}, client).createNewRecoveryCodes({ environmentId: core.environmentId });
};
...
*/
const deleteMfaDevice = async ({ deviceId, mfaAuthToken }, client = require_constants.getDefaultClient()) => {
	const core = require_constants.getCore(client);
	const apiClient = require_constants.createApiClient({}, client);
	require_constants.assertDefined(deviceId, "deviceId is required");
	require_constants.assertDefined(mfaAuthToken, "mfaAuthToken is required");
...
*/
const getMfaDevices = async (client = require_constants.getDefaultClient()) => {
	const core = require_constants.getCore(client);
	const { devices } = await require_constants.createApiClient({}, client).getUserMfaDevices({ environmentId: core.environmentId });
	return devices;
};
...
*/
const getMfaRecoveryCodes = async (client = require_constants.getDefaultClient()) => {
	const core = require_constants.getCore(client);
	return require_constants.createApiClient({}, client).getRecoveryCodes({ environmentId: core.environmentId });
};
...
*/
const isPendingRecoveryCodesAcknowledgment = (client = require_constants.getDefaultClient()) => {
	const user = client.user;
	require_constants.assertDefined(user, "User not logged in");
	return user.mfaBackupCodeAcknowledgement !== _dynamic_labs_sdk_api_core.MfaBackupCodeAcknowledgement.Complete;
};
...
*/
const isUserMissingMfaAuth = (client = require_constants.getDefaultClient()) => {
	const user = client.user;
	require_constants.assertDefined(user, "User not logged in");
...
*/
const registerTotpMfaDevice = async (client = require_constants.getDefaultClient()) => {
	const core = require_constants.getCore(client);
	return require_constants.createApiClient({}, client).registerTotpMfaDevice({ environmentId: core.environmentId });
};
...
*/
const setDefaultMfaDevice = async ({ deviceId }, client = require_constants.getDefaultClient()) => {
	const core = require_constants.getCore(client);
	const apiClient = require_constants.createApiClient({}, client);
	require_constants.assertDefined(deviceId, "deviceId is required");
...
*/
const sendEmailOTP = async ({ email }, client = require_constants.getDefaultClient()) => {
	const core = require_constants.getCore(client);
	const { verificationUUID } = await require_constants.createApiClient({}, client).createEmailVerification({
...
*/
const sendSmsOTP = async ({ isoCountryCode, phoneNumber }, client = require_constants.getDefaultClient()) => {
	const core = require_constants.getCore(client);
	const apiClient = require_constants.createApiClient({}, client);
...
*/
const verifyOTP = async ({ otpVerification, verificationToken }, client = require_constants.getDefaultClient()) => {
...
*/
const deleteUser = async (client = require_constants.getDefaultClient()) => {
	const core = require_constants.getCore(client);
	await require_constants.createApiClient({ includeMfaToken: true }, client).hardDeleteUser({ environmentId: core.environmentId });
	await logout(client);
};
...
*/
const assertWalletAccountSigningAvailability = async ({ walletAccount }, client = require_constants.getDefaultClient()) => {
...
*/
const connectWithWalletProvider = async ({ walletProviderKey, addToDynamicWalletAccounts = true }, client = require_constants.getDefaultClient()) => {
...
*/
const proveWalletAccountOwnership = async ({ walletAccount }, client = require_constants.getDefaultClient()) => {
	const walletProvider = require_getVerifiedCredentialForWalletAccount.getWalletProviderFromWalletAccount({ walletAccount }, client);
...
const verifyWalletAccountForSignInOrTransfer = async ({ walletAccount, isTransfer }, client) => {
	...
	require_constants.assertDefined(walletVerifiedCredential, `After verifying, still unable to find verified wallet credential for wallet account ${walletAccount.address} on chain ${walletAccount.chain}`);
	...
	*/
	...
	require_constants.assertDefined(verifiedWalletAccount, `Unable to find wallet account ${walletAccount.address} on chain ${walletAccount.chain} for verified credential ${walletVerifiedCredential.id}`);
...
*/
const verifyWalletAccount = async ({ walletAccount }, client = require_constants.getDefaultClient()) => {
...
*/
const connectAndVerifyWithWalletProvider = async ({ walletProviderKey }, client = require_constants.getDefaultClient()) => {
...
*/
const getAvailableWalletProvidersData = (client = require_constants.getDefaultClient()) => {
...
*/
const getConnectedAddresses = ({ walletProviderKey }, client = require_constants.getDefaultClient()) => {
...
const getOwnerWalletAccountForSmartWalletAccount = ({ smartWalletAccount }, client = require_constants.getDefaultClient()) => {
	...
	require_constants.assertDefined(smartWalletVerifiedCredential?.signerRefId, "Invalid smart wallet account");
...
*/
const getWalletProviderDataByKey = ({ walletProviderKey }, client = require_constants.getDefaultClient()) => {
...
*/
const getActiveNetworkData = async ({ walletAccount }, client = require_constants.getDefaultClient()) => {
	const walletProvider = require_getVerifiedCredentialForWalletAccount.getWalletProviderFromWalletAccount({ walletAccount }, client);
	const { networkId } = await walletProvider.getActiveNetworkId();
...
*/
const getActiveNetworkId = ({ walletAccount }, client = require_constants.getDefaultClient()) => {
...
*/
const getBalance = async ({ walletAccount }, client = require_constants.getDefaultClient()) => {
	const walletProvider = require_getVerifiedCredentialForWalletAccount.getWalletProviderFromWalletAccount({ walletAccount }, client);
	const { networkId } = await walletProvider.getActiveNetworkId();
...
* @returns A promise that resolves to an object containing the balance as a string or null.
*/
const getBalanceForAddress = async ({ address, chain, networkId }, client = require_constants.getDefaultClient()) => {
	const networkProvider = require_getNetworkProviderFromNetworkId.getNetworkProviderFromNetworkId({
		chain,
		networkId
	}, client);
	require_constants.assertDefined(networkProvider, `No network found for chain ${chain} and network id ${networkId}. Please ensure you've enabled this network in the Dashboard.`);
...
*/
const getTransactionHistory = async (params, client = require_constants.getDefaultClient()) => {
	const core = require_constants.getCore(client);
	return await require_constants.createApiClient({}, client).getWalletTransactions({
...
*/
const isProgrammaticNetworkSwitchAvailable = ({ walletAccount }, client = require_constants.getDefaultClient()) => {
...
*/
const switchActiveNetwork = async ({ networkId, walletAccount }, client = require_constants.getDefaultClient()) => {
	const walletProvider = require_getVerifiedCredentialForWalletAccount.getWalletProviderByKey({ walletProviderKey: walletAccount.walletProviderKey }, client);
...
*/
const selectPrimaryWalletAccount = async ({ walletAccount }, client = require_constants.getDefaultClient()) => {
	const core = require_constants.getCore(client);
	if (getPrimaryWalletAccount(client)?.id === walletAccount.id) return;
	if (walletAccount.verifiedCredentialId) require_getVerifiedCredentialForWalletAccount.updateAuthFromVerifyResponse({ response: await require_constants.createApiClient({}, client).selectUserWallet({
		environmentId: core.environmentId,
...
*/
const signMessage = async ({ walletAccount, message }, client = require_constants.getDefaultClient()) => {
	const walletProvider = require_getVerifiedCredentialForWalletAccount.getWalletProviderFromWalletAccount({ walletAccount }, client);
	assertWalletProviderMethodDefined(walletProvider, "signMessage");
	const { signature } = await walletProvider.signMessage({
		message,
...
*/
const transferWalletAccount = async ({ walletProviderKey }, client = require_constants.getDefaultClient()) => {
	return verifyWalletAccountForSignInOrTransfer({
		isTransfer: true,
...
*/
const getWalletAccountFromAddress = ({ address, chain }, client = require_constants.getDefaultClient()) => {
...
* @param [client] - The Dynamic client instance. Only required when using multiple Dynamic clients.
*/
const offWalletProviderEvent = ({ callback, event, walletProviderKey }, client = require_constants.getDefaultClient()) => {
	const walletProvider = require_getVerifiedCredentialForWalletAccount.getWalletProviderByKey({ walletProviderKey }, client);
...
*/
const onWalletProviderEvent = ({ callback, event, walletProviderKey }, client = require_constants.getDefaultClient()) => {
	const walletProvider = require_getVerifiedCredentialForWalletAccount.getWalletProviderByKey({ walletProviderKey }, client);
	assertWalletProviderMethodDefined(walletProvider, "events");
...
//#endregion
exports.BaseError = require_constants.BaseError;
...
exports.InvalidExternalAuthError = require_constants.InvalidExternalAuthError;
...
exports.LinkCredentialError = require_constants.LinkCredentialError;
...
exports.MfaInvalidOtpError = require_constants.MfaInvalidOtpError;
exports.MfaRateLimitedError = require_constants.MfaRateLimitedError;
...
exports.SandboxMaximumThresholdReachedError = require_constants.SandboxMaximumThresholdReachedError;
...
exports.getDeviceTokenFromUrl = getDeviceTokenFromUrl;
